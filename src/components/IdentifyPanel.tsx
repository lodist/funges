...
/**
 * The photo-identification flow, as a fullscreen overlay.
 *
 * Follows SpeciesSelectorFullscreen's pattern (framer-motion overlay, escape to
 * close, body scroll lock) rather than being a route: it is launched from the map
 * control stack and has no useful URL of its own.
 *
      ...
      <div className='mx-auto max-w-lg p-4 space-y-4'>
        ...
        {phase.name === 'capture' && (
          <section className='space-y-3'>
            ...
            <p className='text-sm text-muted-foreground'>{t('capture.hint')}</p>
            ...
            <button
              ...
              onClick={() => {
                void removeModel().then(() => setPhase({ name: 'needsModel' }));
              }}
            >
              {t('download.remove')}
            </button>
