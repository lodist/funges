...
describe('queryRouteDishData', () => {
  ...
  it('excludes numbers layers, keeping the single forecast fill layer', () => {
    const mapStub = {
      getStyle: () => ({
        layers: [
          {
    ...
    expect(result.sourceGroups[0]?.sourceLayer).toBe('ne_forecast');
  });

  it('reads one fill layer for every day and interpolates scores when frac > 0', () => {
    const mapStub = {
      getStyle: () => ({
        layers: [
          {
      ...
      // The sole tileset carries both endpoints per feature: `_score` (d0) + `_score_d6`.
      querySourceFeatures: () => [
        {
          id: 'stop',
          type: 'Feature',
    ...
    // Day 0 (frac 0): d0 score 6.0 -> covered.
    ...
    // Day 6 (frac 1): same layer, interpolates 6 -> 0, below minScore -> dropped.
    const day6 = queryRouteDishData({ ...common, frac: 1 });
    expect(day6.sourceGroups[0]?.sourceId).toBe('overlay-ne');
