...
from forecast_pipeline import gaussian, altitude_score
...
def _sample(n=3000, seed=11):
    ...
    for d in range(1, 22):                      # 21 temp + 21 humidity lag columns
        t = base_temp + rng.normal(0, 2, n); t[rng.random(n) < 0.08] = np.nan
        h = base_hum + rng.normal(0, 5, n);  h[rng.random(n) < 0.08] = np.nan
