...
from funges_backend.paths import BACKEND_ROOT, REPOSITORY_ROOT
from funges_backend.tools.bioclip_spike import (
    CACHE,
    MODEL_HUB_ID,
    evaluate_embeddings,
    load_model,
...
ARTIFACTS = BACKEND_ROOT / "tools" / "model_artifacts"
...
PUBLIC_MODELS = REPOSITORY_ROOT / "public" / "models"
TEXT_MATRIX = REPOSITORY_ROOT / "src" / "assets" / "bioclip_text_embeddings.f16.bin"
LABELS_TS = REPOSITORY_ROOT / "src" / "data" / "bioclip-labels.ts"
...
SELFCHECK_TS = (
    REPOSITORY_ROOT / "src" / "data" / "bioclip-selfcheck.ts"
)
PROBE_ONNX = PUBLIC_MODELS / "matmulnbits_probe.onnx"
PROBE_TS = REPOSITORY_ROOT / "src" / "data" / "bioclip-probe.ts"
...
def stage_audit_danger():
    ...
    """
    from funges_backend.tools.bioclip_spike import CATALOG_NAMES, CULTIVATED_NAMES, TOXIC_NAMES
...
def _verify_wide(quant_vectors, photos, tier1_labels, tag="int8"):
    ...
    from funges_backend.tools.bioclip_spike import CATALOG_NAMES, _predictions, false_edible_rate
...
def stage_text_matrix():
    ...
    ranks = dict(list(CATALOG) + list(TOXIC))
    ...
    cultivated_ranks = dict(CULTIVATED)
    failed = []
...
PARITY_DIR = REPOSITORY_ROOT / "e2e" / "fixtures" / "bioclip-parity"
...
VARIANT_TS = REPOSITORY_ROOT / "src" / "lib" / "bioclip" / "variant.ts"
...
def stage_upload(only=None):
    ...
    from funges_backend.tools.build_season_curves import get_required_env, load_dotenv
...
SYNTH_DIR = REPOSITORY_ROOT / "src" / "test" / "fixtures"
