...
Usage (from backend/):
  uv run python -m funges_backend.tools.coarsen_coords                 # all regions, LOCAL dry-run
  uv run python -m funges_backend.tools.coarsen_coords NE --km 10      # one region, set spacing
  uv run python -m funges_backend.tools.coarsen_coords --upload        # write _coarse keys to R2
"""
import json
import sys
...
def coarsen_region(name, config, km, *, upload, out_dir):
    ...
    if upload:
        ...
        with open(d / f"{name}_unique_coordinates_coarse.json", "w") as fh:
            json.dump([[f"{a:.{fp.NDP}f}", f"{b:.{fp.NDP}f}"] for a, b in coarse], fh, indent=4)
...
def main():
    ...
    tot_b = sum(r["before"] for r in res)
    tot_a = sum(r["after"] for r in res)
