...
Usage:
  python backend/tools/bake_base_coord_keys.py                 # all regions, LOCAL dry-run
  python backend/tools/bake_base_coord_keys.py NE SE           # subset, LOCAL dry-run
  python backend/tools/bake_base_coord_keys.py --upload        # write back to R2 (prod)
  python backend/tools/bake_base_coord_keys.py NE --out-dir /tmp/baked
...
from funges_backend.regions.config import REGION_CONFIGS as REGIONS
...
def main():
    ...
    fp.load_dotenv(REPOSITORY_ROOT / ".env")
    fp.load_dotenv(REPOSITORY_ROOT / ".env.secret")
