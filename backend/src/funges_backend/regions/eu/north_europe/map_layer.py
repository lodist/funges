...
def split_geojson_by_size(geo_df, max_size_mb=5):
    ...
    for index, _row in geo_df.iterrows():
        ...
        if current_size + row_size > max_size_mb * 1024 * 1024 and current_subset:
...
species_forest_mapping.update(LANDCOVER_BY_REGION["NE"])


print("Fetching weather scores and habitats from Postgres...")
df, clipped_triangles_geojson = load_map_inputs("NE")
print("Weather data fetched successfully!")

df["Date"] = pd.to_datetime(df["Date"])
full_df = (
    df.copy()
)  # forward window (today..+6) for the forecast tileset; today path collapses df below
...
clipped_triangles_geojson["geometry"] = clipped_triangles_geojson.geometry.buffer(
    0
)  # Clean geometries
...
clipped_tri_gdf = gpd.sjoin(
    clipped_tri_gdf,
    clipped_triangles_geojson[["raster_val", "geometry"]],
    how="left",
...
matched_triangles = gpd.sjoin(
    clipped_tri_gdf, tri_gdf[["tri_id", "geometry"]], how="inner", predicate="intersects"
)
...
for specie, valid_rasters in species_forest_mapping.items():
    ...
    clipped_tri_gdf[score_col] = clipped_tri_gdf["raster_val"].apply(
        lambda val, valid=valid_rasters: np.nan if pd.notna(val) and val in valid else 0.0
    )
...
_dom = (
    _clipped_proj.dropna(subset=["raster_val"])
    .groupby(["tri_id", "raster_val"], as_index=False)["__area_m2"]
    .sum()
)
...
tree = cKDTree(xy_m)
_canon_latlon = list(
    zip(gdf["Latitude"].round(3).to_numpy(), gdf["Longitude"].round(3).to_numpy())
)  # forecast: align per-day point scores to xy_m order
...
tri_centroids_m = {
    int(row.tri_id): np.array(
        transformer.transform(row.geometry.centroid.x, row.geometry.centroid.y), dtype=float
    )
...
_tri_geom = (
    clipped_tri_gdf.drop_duplicates(subset=["tri_id"]).set_index("tri_id")["geometry"].apply(drop_z)
)  # forecast: tri_id -> clean geometry
...
def upload_mbtiles_to_r2(mbtiles_path: Path, r2_key: str) -> None:
    client, bucket_name = r2_output_client()
    print(f"Uploading {mbtiles_path.name} to R2: {r2_key}...")
    with open(mbtiles_path, "rb") as f:
        client.put_object(
            Bucket=bucket_name, Key=r2_key, Body=f.read(), ContentType="application/octet-stream"
        )
...
with tempfile.TemporaryDirectory() as tmpdir:
    ...
    fc_gdf = (
        gpd.GeoDataFrame(fc_features, crs=gdf_full.crs)
        if fc_features
