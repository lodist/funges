...
region_code = 'ne'
REGION = "NE"
REGION_LAT_RANGE = (49.0, 71.5)
REGION_LON_RANGE = (-25.0, 32.0)
...
def split_geojson_by_size(geo_df, max_size_mb=5):
    ...
    for index, _row in geo_df.iterrows():
        ...
        if current_size + row_size > max_size_mb * 1024 * 1024 and current_subset:
            # store the current subset and start a new one
...
CLIPPED_TRIANGLES_PATH = get_required_env("NE_CLIPPED_GPKG")
...
for specie, valid_rasters in species_forest_mapping.items():
    ...
    clipped_tri_gdf[score_col] = clipped_tri_gdf['raster_val'].apply(
        lambda val, valid_rasters=valid_rasters: np.nan if pd.notna(val) and val in valid_rasters else 0.0
    )
...
tri_id_to_raster = dict(zip(_dominant["tri_id"], _dominant["raster_val"], strict=True))
...
tree       = cKDTree(xy_m)
_canon_latlon = list(zip(gdf['Latitude'].round(3).to_numpy(), gdf['Longitude'].round(3).to_numpy(), strict=True))  # forecast: align per-day point scores to xy_m order
...
def _combine(values: np.ndarray, dists_m: np.ndarray) -> float:
    ...
    if n == 0:
        return np.nan
    w = (np.exp(-(dists_m**2) / (2.0 * sigma_m**2)) if use_gaussian
         else 1.0 / np.maximum(dists_m, 1.0)**p_idw)
    if n >= 20:
        k = max(5, int(0.3 * n))
...
for tri_id in tri_ids:
    ...
    neigh_w = (np.exp(-(dists_m**2) / (2.0 * sigma_m**2)) if use_gaussian
               else 1.0 / np.maximum(dists_m, 1.0)**p_idw)
