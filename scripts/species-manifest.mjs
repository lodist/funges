...
const REGION_SUFFIXES = REGIONS.map(region => region.toLowerCase());
// Single source of truth for which region a coordinate belongs to. Generated into
// src/generated/species-catalog.ts (frontend) and backend/generated/species_registry.json
// (pipeline), so the TS map store and the Python scoring scripts cannot drift apart.
// REGION_BBOX in the map store is a deliberately coarser, separate thing: viewport
// tile-activation boxes, not these boundaries.
const REGION_BOUNDARIES = {
  uswMaxLongitude: -100,
  usMaxLongitude: -25,
  seMaxLatitude: 47,
};
// The style layer every generated forecast layer is cloned from. Declared, not
// inferred: "the first species layer that happens to exist" would silently spread a
...
export function validateManifests(
  ...
  for (const { directory, filename, data: m } of entries) {
    ...
    if (forecast?.enabled) {
      if (typeof forecast.routeToDish !== 'boolean')
        fail(
          errors,
          where,
          'forecast.routeToDish must be boolean when forecast is enabled'
        );
  ...
  // The style generator clones every forecast layer from this species' layers, so
  // it has to stay a forecast species available everywhere: were it disabled in a
  // region, generate would orphan-delete the template it needs on the next run.
  if (entries.some(x => x.data.forecast?.enabled)) {
    const template = entries.find(x => x.data.id === STYLE_TEMPLATE_SPECIES);
    if (
      !template?.data.forecast?.enabled ||
      !REGIONS.every(
        region => template.data.forecast.regions?.[region]?.available
      )
    )
      fail(
        errors,
        'content/species',
        `${STYLE_TEMPLATE_SPECIES} is the declared style template: it must be a forecast species available in every region`
      );
  }
  return errors;
}
...
function catalogTs(manifests) {
  const rows = manifests.map(m => ({
    ...
    ...(m.forecast.enabled
      ? {
          showOnMap: true,
          forecastRegions: REGIONS.filter(
            region => m.forecast.regions[region].available
          ),
        }
      : {}),
  }));
  return `// GENERATED by npm run species:generate. DO NOT EDIT.\nimport type { Species } from '@/data/species';\n\nexport const GENERATED_SPECIES_DATA: Species[] = ${JSON.stringify(rows, null, 2)};\n\nexport const GENERATED_REGION_BOUNDARIES = ${JSON.stringify(REGION_BOUNDARIES, null, 2)} as const;\n`;
}

function routeToDishTs(manifests) {
  const config = Object.fromEntries(
    manifests
      .filter(m => m.forecast.enabled && m.forecast.routeToDish)
      .map(m => [
        m.id,
        {
...
function backendRegistry(manifests) {
  ...
  return { schemaVersion: 1, regionBoundaries: REGION_BOUNDARIES, species };
}
...
// A forecast species layer is `<id>_<region>` or `<id>_<region>_numbers` on one of
// the overlay sources. Both halves of that test matter: the id shape spares other
// layers that share those sources (masks, outlines, legends), the source spares
// basemap layers that happen to end in a region suffix.
function isSpeciesLayerId(id) {
  return REGION_SUFFIXES.some(
    region => id.endsWith(`_${region}`) || id.endsWith(`_${region}_numbers`)
  );
}

function reconcileStyleLayers(style, wantedIdsByRegion) {
  const out = structuredClone(style);
  const existing = new Map(out.layers.map(layer => [layer.id, layer]));
  const templates = new Map();
  for (const region of REGION_SUFFIXES) {
    const base = existing.get(`${STYLE_TEMPLATE_SPECIES}_${region}`);
    const numbers = existing.get(`${STYLE_TEMPLATE_SPECIES}_${region}_numbers`);
    if (!base || !numbers) {
      if (wantedIdsByRegion[region.toUpperCase()].length)
        throw new Error(
          `style is missing the ${STYLE_TEMPLATE_SPECIES}_${region} forecast template layers`
        );
      continue;
    }
    templates.set(region, { base, numbers });
  }
  if (!templates.size) return out;

  const overlaySources = new Set(
    [...templates.values()].map(({ base }) => base.source)
  );
  const isSpeciesLayer = layer =>
    overlaySources.has(layer.source) && isSpeciesLayerId(layer.id);
  // Everything before the first species layer is a non-species layer, so this index
  // is also where the block belongs inside `rest` below.
  const insertion = out.layers.findIndex(isSpeciesLayer);
  if (insertion < 0)
    throw new Error('style is missing forecast overlay layers');

  // Rebuild the whole species block in manifest order, so the layer list is a
  // function of the manifests alone: existing layers keep their paint, missing ones
  // are cloned from the region template, and a disable -> enable cycle can no longer
  // shuffle a species to the end. Fills stay below the numbers symbols, and the block
  // stays where it already sat, so basemap paint order is untouched.
  const fills = [];
  const numbers = [];
  const clone = (template, id) =>
    replaceDeep(template, `${STYLE_TEMPLATE_SPECIES}_`, `${id}_`);
  const orderedIds = [
  ...
  for (const id of orderedIds) {
    for (const region of REGION_SUFFIXES) {
      ...
      const template = templates.get(region);
      fills.push(existing.get(`${id}_${region}`) ?? clone(template.base, id));
      numbers.push(
        existing.get(`${id}_${region}_numbers`) ?? clone(template.numbers, id)
      );
    }
  }
  const rest = out.layers.filter(layer => !isSpeciesLayer(layer));
  out.layers = [
...
function usage() {
  console.log(
    'Usage: species-manifest.mjs <scaffold --id ID [--forecast]|generate|check>'
  );
}
  ...
  try {
    if (command === 'generate') {
      if (process.argv.includes('--id'))
        throw new Error('--id is only supported by scaffold');
      await generate();
