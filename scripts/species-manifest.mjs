...
function reconcileStyleLayers(style, forecastIdsByRegion) {
    ...
    const templateId =
      ...
      out.layers
        .find(
          layer =>
            layer.type === 'fill' &&
            layer.id.endsWith(`_${region}`) &&
            out.layers.some(other => other.id === `${layer.id}_numbers`)
        )
        ?.id.slice(0, -`_${region}`.length);
    if (!templateId) {
      if (wantedIds.length)
        throw new Error(`style is missing forecast templates for ${region}`);
      continue;
    }
    const templateBase = out.layers.find(
      layer => layer.id === `${templateId}_${region}`
    );
    const templateNumbers = out.layers.find(
      layer => layer.id === `${templateId}_${region}_numbers`
    );
    if (!templateBase || !templateNumbers)
      throw new Error(`style is missing forecast templates for ${region}`);
    templates.set(region, {
      templateBase,
      templateNumbers,
      templatePrefix: `${templateId}_`,
    });
  }
  ...
  const overlaySources = new Set(
    [...templates.values()].map(({ templateBase }) => templateBase.source)
  );
  const originalInsertion = out.layers.findIndex(layer =>
    overlaySources.has(layer.source)
  );
  if (originalInsertion < 0)
  ...
  out.layers = out.layers.filter(
    layer => !overlaySources.has(layer.source) || wantedLayerIds.has(layer.id)
  );
  ...
  for (const id of orderedIds) {
    for (const region of REGIONS.map(x => x.toLowerCase())) {
      ...
      if (!existingLayers.has(`${id}_${region}`))
        fills.push(replaceDeep(templateBase, templatePrefix, `${id}_`));
      if (!existingLayers.has(`${id}_${region}_numbers`))
        numbers.push(replaceDeep(templateNumbers, templatePrefix, `${id}_`));
    }
  }

  const firstSymbol = out.layers.findIndex(
    layer => overlaySources.has(layer.source) && layer.type === 'symbol'
  );
  const fillInsertion = firstSymbol < 0 ? originalInsertion : firstSymbol;
  out.layers.splice(fillInsertion, 0, ...fills);
  const lastOverlay = out.layers.findLastIndex(layer =>
    overlaySources.has(layer.source)
  );
  out.layers.splice(lastOverlay + 1, 0, ...numbers);
  return out;
}
