...
**Ask:** `$impeccable ask` maps a situation onto the command or sequence that fits, then stops without running it. Load [reference/ask.md](reference/ask.md) when the user invokes it, asks which command applies, asks where to start, or asks what order to run things in. A request for design work stays design work; this fires on questions about the map, not on the work itself.
